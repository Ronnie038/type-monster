...
// FINISHED TYPING
const gameOver = (userText) => {
	document.removeEventListener('keydown', typeController);
	// the current time is the finish time
	// so total time taken is current time - start time
	const finishTime = new Date().getTime();
	const timeTaken = (finishTime - startTime) / 1000;
	let userTextSplitedInArray = userText.split(' ');
	let wpmEquation = parseInt(timeTaken) / 60;
	let wpm = parseInt(userTextSplitedInArray.length / wpmEquation);
	wpmDiv.innerHTML = `<span>WPM : ${wpm}</span>`;
